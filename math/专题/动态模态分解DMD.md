...
SVD为分解出各个基态 U， 表达每个基态能量的  $\Sigma$ ，表达每个基态随着时间变化的情况 V
降到多少维度是由人为指定，这里降到r 
$$
...
当我场内测量点较少且每个时间点得到的数据列出稀疏矩阵时候采用；
$$
y = \Phi x=\Phi\Psi a=\Omega a
\\
y底维度的测量数据
\\
\Phi随机测量矩阵
...
\Psi 基态矩阵
$$
即随机抽取场内的速度，构造傅里叶系数向量



## 压缩DMD技术

当我有具有非常高精度的大量稠密数据的时候可采用压缩DMD技术；

由于对原始数据X做SVD较为消耗算力，则可以在X场中随机抽取数据，当作一个较小的压缩了的场X'，然后做DMD
