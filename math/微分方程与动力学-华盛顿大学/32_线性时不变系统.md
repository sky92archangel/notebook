...
%% simulate forced pendulum in down position
A = [0 1; -1 -0.1];  % added small damping (-0.1 omega)
B = [0; 1];
C = eye(2);
D = [0; 0];
...
%% linear response to arbitrary input
t = 0:0.1:50;
u = 0*t;
...
# define the dynamical matrices
A = np.array([[0, 1], [-1, -0.1]])  # added small damping (-0.1 omega)
B = np.array([[0], [1]])
C = np.eye(2)
D = np.array([[0], [0]])

sys = StateSpace(A, B, C, D)  # make a state space model

# 修复：移除错误的转置操作，保留impulse返回的原始维度
t_impulse = np.linspace(0, 100, 1000)  # 0到100秒，1000个时间点
_, y_impulse = impulse(sys, T=t_impulse)  # y_impulse形状为(1000, 2)，直接匹配t_impulse

# send an impulse to the system and see the system "ring" and then decay
plt.plot(t_impulse, y_impulse)
plt.grid(True)
plt.title("Impulse response of the system")
plt.legend(['State 1', 'State 2'])  # 增加图例区分两个状态
plt.show()

# linear response to arbitrary input (triangular pulse)
t = np.arange(0, 50.1, 0.1)
u = np.zeros_like(t)
# triangular pulse from t=10s to t=30s
u[100:200] = np.linspace(0, 0.1, 100)
u[200:300] = np.linspace(0.1, 0, 100)

plt.figure()
plt.plot(t, u)
plt.title("Input signal (triangular pulse)")
plt.grid(True)
plt.show()

t_lsim, y_lsim, _ = lsim(sys, U=u, T=t)
plt.figure()
plt.plot(t_lsim, y_lsim)
plt.title("System response to triangular pulse")
plt.legend(['State 1', 'State 2'])  # 增加图例
plt.grid(True)
plt.show()
```
...
![image-20260309181005547](32_线性时不变系统.assets/image-20260309181005547.png)
